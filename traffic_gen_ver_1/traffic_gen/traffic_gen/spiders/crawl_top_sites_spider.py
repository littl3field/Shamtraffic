...
class CrawlTopSitesSpider(scrapy.Spider):
    name= 'crawltopsites'                               #This is the name that you use to refer to this spider
    file_of_sites = open(globalVariables.THOUSAND_TOP_SITES_FILE,'r')     #The file which holds the top 1000 most popular sites
    ...
    def adjustTime(self, time):
        if (time > 9 and time < 10):
            ...
            self.custom_settings['DOWNLOAD_DELAY'] = globalVariables.OFF_PEAK_DELAY
    ...
    def saveDownloads(self, response):
        ...
        with open(os.path.normpath(os.path.join(os.path.dirname(__file__),os.path.pardir,'downloaded_files' + new_url)),"wb") as f:    #Set the file path for the downloaded documents here
